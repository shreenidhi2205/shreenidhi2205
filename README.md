<h1 align="center">Hi 👋, I'm Shreenidhi Rithe</h1>
<p align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35">
</p>
<p align="center">
  🎓 Computer Engineering Student | 💻 Full Stack Developer | 🤖 AI/ML Enthusiast | 🔗 Exploring Blockchain
</p>
<p align="center">
  📍 Pune, India
</p>

---

## 🧑‍💻 About Me

I am a Computer Engineering student (SPPU, CGPA: 8.75) passionate about building practical and scalable tech solutions.

- 💡 Strong in Data Structures & Algorithms (Java)
- 🌐 Full Stack Web Development
- 🤖 Exploring Machine Learning
- 🔗 Learning Blockchain Fundamentals
- 🚀 Focused on internships & placements

---

# 🛠️ Technologies & Tools

<p align="center">
<img src="https://skillicons.dev/icons?i=java,spring,python,javascript,react,nodejs,express,mongodb,mysql,firebase,git,github,postman,figma,vscode" />
</p>

---

# 🚀 Featured Projects

### ⬡ GitLite — Git-Inspired Version Control System
> Java · Spring Boot · MySQL · React · REST API

A version control backend implementing core Git internals from scratch.

- SHA-1 content-addressable blob storage with deduplication
- Parent-linked linear commit history and branch pointer management
- Layered MVC architecture with multi-repo data isolation
- Commit-level file preview and download via commit-to-blob traversal
- Deployed on Railway (backend) & Vercel (frontend)

🔗 [Live Demo](https://git-lite.vercel.app) · [GitHub](https://github.com/shreenidhi2205/GitLite)

---

### ♻️ E-Waste Management Platform
> AI + Blockchain integrated full-stack solution for secure e-waste disposal.

- Vendor & User Login
- AI Product Analysis
- AR Visualization
- Blockchain Tracking
- Pickup Scheduling

---

### 📚 Decentralised Library Platform
> Blockchain-based secure book sharing system with MetaMask integration.

---

### 🧠 Psychology Quiz Website
> Real-time quiz with dynamic certificate generation & admin dashboard.

---

# 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=shreenidhi2205&show_icons=true&theme=tokyonight" />
</p>
<p align="center">
  <img src="https://streak-stats.demolab.com?user=shreenidhi2205&theme=tokyonight" />
</p>
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=shreenidhi2205&layout=compact&theme=tokyonight" />
</p>

---

# 🏆 Achievements

🥇 Chess Competition Winner – Elpro Sports
🇩🇪 Fit-1 German Certification (50/60)
📄 Research Paper Published – Weather Display Platform
🌸 GirlScript Summer of Code – Open Source Contributor

---

# 📜 Certifications

- NPTEL – Programming in Java
- PW Skills – Backend Development
- Great Learning – Python Fundamentals
- IIT Madras – Decentralised Ledger Workshop

---

# 🤝 Let's Connect

<p align="center">
  <a href="https://www.linkedin.com/in/shreenidhi-rithe-272826292/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:shreenidhirithe2210@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

# 🎯 2026 Goals

- Crack top tech internship
- Master DSA patterns
- Build 2 advanced AI-integrated systems
- Contribute to Open Source

---

<p align="center">
  ⭐️ Always open to collaboration & innovative ideas!
</p>
